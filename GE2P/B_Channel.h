...
public:
	...
	int McBSP_IO( int data )
	{
		...
		else
		{
			...
			data = _lsshl( _lsmpy( -TxBuf[ bufp ][ curp ] << 3, TX_amp ), 7 ) >> 16;
			}
...
class B_Channel
{
	...
	struct RequestedParameters
	{
		...
		unsigned int VRVAD_Threshold; // exp avg abs PCM value
		unsigned int VRVAD_HangoverTime; // ms
		};
...
public:
		...
		, D_Ch( NULL )
	{
		...
		param.VRVAD_HangoverTime = 5000; // ms
		}

	void Initialize( ELU28_D_Channel* owner, bool ALaw = true );
